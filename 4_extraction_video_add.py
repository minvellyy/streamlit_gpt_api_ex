...
def diarize_speakers(audio_path, hf_token):
    ...
    if not _HAS_PYANNOTE:
        raise RuntimeError("pyannote.audio가 설치되지 않음")
...
def align_segments(whisper_data, spk_segments):
    """Whisper 세그먼트와 화자 구간 매칭"""
    w_segments = whisper_data.get("segments", [])
    aligned = []
    ...
    for seg in w_segments:
        ...
        w_end = float(seg.get("end", 0.0))
        text = seg.get("text", "").strip()
        
        best_speaker, best_overlap = "SPK-UNK", 0.0
        j = max(0, i - 5)
        
        while j < n:
            s_start, s_end, spk = spk_segments[j]
            if s_start > w_end:
                break
...
def prettify_speaker_labels(aligned):
    """화자 라벨을 A/B/C로 변환"""
    mapping = {}
    next_code = 0
    label_table = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    
    for item in aligned:
        raw = item["speaker"]
        if raw not in mapping:
            label = f"화자 {label_table[next_code % len(label_table)]}"
            mapping[raw] = label
            next_code += 1
...
def build_speaker_script(aligned):
    """화자별 스크립트 생성"""
    if not aligned:
        return ""
    ...
    def flush():
        if not buffer_texts:
            return
        lines.append(f"{last_speaker}: " + " ".join(buffer_texts))
    
    for seg in aligned:
        spk, txt = seg["speaker"], seg["text"]
        if spk != last_speaker:
            flush()
            last_speaker = spk
            buffer_texts = [txt] if txt else []
        else:
            if txt:
...
def main():
    ...
    st.markdown("*비디오 → 오디오 추출 → 전사 → 번역 → 이미지 생성 → 키워드 추출 → 화자 분리*")
    ...
    if uploaded_file is not None:
        ...
        # === 6. 화자 분리 (선택) ===
        st.markdown("---")
        with st.expander("🔊 화자 분리 (Speaker Diarization)"):
            if not _HAS_PYANNOTE:
                st.warning("`pyannote.audio` 미설치. `pip install pyannote.audio torch` 필요")
            else:
                hf_token = st.text_input("Hugging Face Token", type="password", placeholder="hf_xxx...")
                run_diar = st.button("화자 분리 실행")
                
                if run_diar and hf_token:
                    try:
                        with st.spinner("화자 구간 분석 중..."):
                            detailed = transcribe_audio_with_timestamps(audio_path, client)
                            spk_segments = diarize_speakers(audio_path, hf_token)
                            aligned = align_segments(detailed, spk_segments)
                            aligned = prettify_speaker_labels(aligned)
                        
                        speaker_script_txt = build_speaker_script(aligned)
                        srt_text = to_srt(aligned)
                        
                        st.success("✅ 화자 분리 완료")
                        st.text_area("화자별 스크립트", speaker_script_txt, height=250)
                        
                        col_s1, col_s2 = st.columns(2)
                        with col_s1:
                            st.download_button("📥 화자 스크립트", speaker_script_txt.encode("utf-8"), "speaker_script.txt")
                        with col_s2:
                            st.download_button("📥 SRT 자막", srt_text.encode("utf-8"), "subtitles.srt")
                    except Exception as e:
                        st.error(f"화자 분리 오류: {e}")
